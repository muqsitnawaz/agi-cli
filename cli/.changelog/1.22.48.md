- **`agents daemon services` now shows live health for every registered service, and `enable`/`disable`/`restart` apply live (RUSH-3193 P4).** Before, the command only probed the two hosted sockets (secrets broker, browser IPC); the 10 other services (the routine scheduler, webhook receiver, watchdog, and the `ServiceSupervisor`-managed secrets-broker/browser-ipc/account-state/session-index/monitors) were invisible, and `enable`/`disable` only wrote `services.yaml` and told the operator to run `agents daemon reload` themselves. Now every registered service renders a row — state (`running`/`parked`/`stopped`, measured for the 5 supervisor-managed services, inferred for the rest), enabled, consecutive failures, last error — and `services --json` carries the same roster additively alongside the pre-existing `secretsBroker`/`browserIpc` fields (unchanged, for existing agent/CI consumers). `agents daemon services enable|disable <id>` now signals the running daemon over the same `SIGHUP` `agents daemon reload` uses, and the daemon's reload handler drives `supervisor.start`/`stop(id)` live for the 5 supervised services — no restart. New: `agents daemon services restart <id>` forces an immediate live restart the same way. A service disabled at daemon boot (never registered on the supervisor) and the 7 not-yet-migrated services still need `agents daemon restart` — both the CLI output and `--json`'s `supervised` field say so per row. Source: `cli/src/lib/daemon/supervisor.ts`, `cli/src/lib/daemon-health.ts`, `cli/src/lib/daemon-services.ts`, `cli/src/lib/daemon/daemon.ts`, `cli/src/commands/daemon.ts`.
