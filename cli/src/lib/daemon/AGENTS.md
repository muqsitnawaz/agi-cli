...
- **Four hand-rolled cross-device distribution patterns** (fanout / mirror /
  provider / elected-singleton, described above) formalized as declared
  `placement` (`every-device` | `singleton`) and `distribution` (`fanout` |
  `mirror` | `provider` | `local`) fields resolved by shared helpers, instead
  of four independent implementations.
