...
| CLI release | [`apps/cli/scripts/release.sh`](apps/cli/scripts/release.sh) `<version> [--apply] [--home-base <host>]` | zero-config self-routing publish of `@phnx-labs/agents-cli` to npm: runnable from any fleet box with an empty environment — tests on a dynamic crabbox, PR + CI, then build/sign/notarize/publish on the `mac-mini` home base (the one hardcoded name), or another Mac via `--home-base` when that box is unreachable; prints a `[n/6]` phase tracker. Legacy `@swarmify` shim built for reference, not published |
