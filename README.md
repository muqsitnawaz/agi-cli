...
| [`apps/ext`](apps/ext) | **Factory** — a VS Code extension that spawns agent terminals as tabs and adds the Factory Floor dashboard. A separate product with its own publish identity. |
...
| [`packages/session-tracker`](packages/session-tracker) | The `SessionStart` hook that writes live-session state the **Factory extension** reads back (`apps/ext/src/core/liveSession.ts`) — not the CLI, which reads transcripts. |
