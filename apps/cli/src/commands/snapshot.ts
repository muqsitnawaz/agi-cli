 ...
 *
 * Replaces the N× `view --json` + session-list/live joins (+ optional feed)
 * with one canonical inventory + active + resumable-session read. Does NOT replace `agents status`, which
...
export function registerSnapshotCommand(program: Command): void {
  const cmd = addHostOption(
    program
      .command('snapshot')
      .description(
        'One-process poll snapshot: install inventory + active and resumable sessions (optional feed/sync). Not the sync-status command — use `agents status` for drift.',
      ),
  )
  ...
  setHelpSections(cmd, {
    examples: `
      ...
      # Fleet-wide active + resumable sessions
