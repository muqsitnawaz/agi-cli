...
describe('doctor qualifier resolution via subprocess (issue #2058)', () => {
  ...
  it('an exact version resolves through the shared resolver (not the bare sweep)', () => {
    // The exact selector `claude@2.0.0` now routes through resolveAgentTargets
    // (doctor.ts parseTargetArg), same as the symbolic qualifiers — it must
    // resolve to that single version, not fall through to a not-installed error.
    seedHome(['2.0.0', '2.1.0'], '2.0.0');
