...
function registerDevicesCommands(program: Command): void {
  ...
  const runList = async (opts: { json?: boolean; stats?: boolean; full?: boolean; refresh?: boolean; live?: boolean; all?: boolean } = {}) => {
    ...
    if (opts.json) {
      // Registry + per-device operator docs, always fast — the Factory
      // extension polls this path. Each row is the EFFECTIVE profile (registry
      // overlaid with the ssh.*/platform/user config layers) and carries its
      // device-layer `config` (maxAgents, schedulerEnabled, notes, ssh*,
      // autoLaunch* — keys set ON that device only) plus an `interactive` flag
      // for the configured interactive host.
