...
export async function resolveRemotePath(
  ...
  if (rawPath === '~' || rawPath.startsWith('~/')) {
    ...
    return rawPath === '~' ? home : home + rawPath.slice(1);
  }
  if (rawPath === '$HOME' || rawPath.startsWith('$HOME/')) {
    const home = await resolveRemoteHome(target, extraSshArgs);
    return rawPath === '$HOME' ? home : home + rawPath.slice(5);
  }
  return rawPath;
}
...
export function registerCpCommand(program: Command): void {
  ...
  setHelpSections(cmd, {
    examples: `
      agents cp yosemite-s0:/abs/data.json ./       # download a file from a fleet host
      agents cp ./build.tar.gz mac-mini:/abs/deploy/ # upload a local file to a fleet host
      agents cp -r yosemite-s0:/abs/src/ mac-mini:/abs/dst/ # copy a directory between two fleet hosts
      agents cp yosemite-s0:~/logs/run.log /tmp/    # ~ is resolved on the remote host
    `,
    notes: `
