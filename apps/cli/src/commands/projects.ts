 ...
 */
function originRemoteUrl(cwd: string): string | undefined {
  try {
    return (
...
export function originSlug(cwd: string): string | undefined {
  const url = originRemoteUrl(cwd);
  return url ? parseOwnerRepoFromRemote(url) ?? undefined : undefined;
}
...
export function repoForDir(dir: string, slugOverride?: string): ProjectRepo {
  ...
  if (slugOverride) return { slug: slugOverride, path: abs };
  const url = originRemoteUrl(abs);
  if (!url) {
    throw new Error(
  ...
  const slug = parseOwnerRepoFromRemote(url) ?? undefined;
  if (!slug) {
    // The dir IS a git repo with an origin — it just is not a GitHub owner/repo.
    // Diagnose that precisely instead of the misleading "not a git repo".
    throw new Error(
      `Cannot resolve a repo slug for "${dir}": its origin (${url}) is not a GitHub owner/repo. ` +
...
/**
 * Merge `added` bound-repo entries into `existing`, keyed by (path, subpath): an
 * exact-location match refreshes the slug in place, everything else is
 * appended. Preserves every existing entry — never collapses two bindings that
 * share a path but differ in subpath. Used by both `add` (against an empty
 * base, so intra-flag dups collapse) and `set --add-dir`.
 */
export function mergeBoundDirs(existing: ProjectRepo[], added: ProjectRepo[]): ProjectRepo[] {
  const result = [...existing];
  for (const r of added) {
    const k = repoLocationKey(r);
    const idx = result.findIndex((e) => repoLocationKey(e) === k);
...
export function registerProjectsCommands(program: Command): void {
  ...
  setHelpSections(projects, {
    ...
    notes: `
      ...
      removed). AGI EXT reads and writes only through these commands — never
  ...
  projects
    ...
    .action(
      async (
        ...
        if (opts.dir?.length) {
          try {
            // Merge against an empty base so a repeated `--dir X --dir X`
            // collapses, matching `set --add-dir`'s dedup.
            def.repos = mergeBoundDirs([], reposFromDirFlags(opts.dir, opts.slug));
          } catch (e) {
            console.error(chalk.red(e instanceof Error ? e.message : String(e)));
            process.exit(1);
          }
  ...
  projects
    ...
    .action((name: string, opts: { repo?: string; root?: string; path?: string; description?: string; goal?: string[]; addDir?: string[]; rmDir?: string[]; slug?: string }) => {
      ...
      if (opts.goal?.length) def.goals = opts.goal.map(parseGoalFlag);
      // Bound-repo dirs: remove first, then add. Keying is on (path, subpath),
      ...
      if (opts.addDir?.length) {
        ...
        def.repos = mergeBoundDirs(def.repos ?? [], entries);
      }
