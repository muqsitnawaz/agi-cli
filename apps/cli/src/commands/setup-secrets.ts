 ...
 *
 * The wizard writes setup preferences and uses the existing secrets command
 * surface for imports. It does not reach into bundle storage internals.
 */
...
function backendArgs(backend: SetupSecretsBackend): string[] {
  if (backend === 'vault') return ['--synced'];
...
function runAgentsSubcommand(args: string[]): void {
  ...
  const res = spawnSync(process.execPath, [entry, ...args], { stdio: 'inherit', env: process.env });
  if ((res.status ?? 1) !== 0) {
...
async function maybeImportSecrets(
  ...
  if (!bundle) {
    if (!isInteractiveTerminal()) throw new Error(`--bundle is required with --import-from ${source} in a non-interactive shell.`);
  ...
  if (source === 'env') {
    ...
    if (!envPath) {
      if (!isInteractiveTerminal()) throw new Error('--vault must name the .env path when --import-from env is used non-interactively.');
      const { input } = await import('@inquirer/prompts');
      envPath = await input({
