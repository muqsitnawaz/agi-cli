...
export function buildExecEnv(options: ExecOptions): NodeJS.ProcessEnv {
  ...
  if (options.agent === 'claude') {
    ...
    // Cursor's OAuth login is device-global — browser sign-in is one account per
    // device. Api-key accounts inject CURSOR_API_KEY via options.env (account
    // resolver path) which wins here at return time.
    delete result.CLAUDE_CONFIG_DIR;
    delete result.CODEX_HOME;
    delete result.COPILOT_HOME;
    delete result.KIMI_CODE_HOME;
