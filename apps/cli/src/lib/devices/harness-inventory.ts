...
export interface QuotaSummary {
  ...
  stale: boolean;
  /** Epoch ms of the underlying snapshot, or null when no snapshot exists. */
  capturedAt?: number | null;
  /** Why quota could not be read. Null means no provider error was reported. */
...
export interface HarnessRow {
  ...
  reason?: string;
  /** Epoch ms when this eligibility verdict was computed. */
  capturedAt?: number;
}
...
export function summarizeQuota(
  ...
  if (!snapshot || snapshot.windows.length === 0) {
    return { status: accountStatus, usedPercent: null, stale: false, capturedAt: null, unavailableReason };
  }
  const blocking = snapshot.windows.filter((w) => w.key !== 'sonnet_week');
  const windows = blocking.length > 0 ? blocking : snapshot.windows;
  const status = accountStatus === 'out_of_credits'
    ? 'out_of_credits'
    : deriveUsageStatusFromSnapshot(snapshot);
...
export async function collectLocalHarnessInventory(opts?: {
  ...
  const usageByKey = usageInputs.length
    ? (await getUsageInfoByIdentity(usageInputs, { readOnly: !opts?.refresh })).usageByKey
    : new Map();
  ...
  return pending.map(({ agent, version, info }) => {
    ...
    const quota = summarizeQuota(snapshot, usage?.error ?? null, info?.usageStatus);
    ...
    return { agent, version, account, signedIn, quota, ready, reason, capturedAt: Date.now() };
  });
}
...
function quotaPaint(quota: QuotaSummary): (s: string) => string {
  if (quota.status === 'rate_limited' || quota.status === 'out_of_credits') return chalk.red;
