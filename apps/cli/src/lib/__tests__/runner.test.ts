...
describe('buildRoutineSpawnEnv', () => {
  ...
  // The daemon holds/injects no token of its own: buildRoutineSpawnEnv delegates
  // Claude auth entirely to buildExecEnv, which resolves the account's file-backed
  // setup-token on the standard path (per-account resolution is covered by
  // account-token.test.ts). No RUSH-1979 ambient-token drop dance survives.
  describe('routine auth is delegated to the standard path (no daemon-special token handling)', () => {
    it('does not strip a caller-provided CLAUDE_CODE_OAUTH_TOKEN (no band-aid drop)', () => {
      const VER = '9.9.9-routine-auth-test';
      const env = buildRoutineSpawnEnv(
        { HOME: '/tmp/overlay', PATH: '/usr/bin', CLAUDE_CODE_OAUTH_TOKEN: 'caller-provided' },
        'claude',
        VER,
      );
      // buildExecEnv pins the version config dir and leaves an explicitly provided
      // token intact; it never conditionally deletes it based on on-disk creds.
      expect(env.CLAUDE_CONFIG_DIR).toContain(path.join('claude', VER));
      expect(env.CLAUDE_CODE_OAUTH_TOKEN).toBe('caller-provided');
    });
  });
});
