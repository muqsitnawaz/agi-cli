...
describe('KNOWN_TOP_LEVEL_COMMANDS', () => {
  ...
  it('does not recognize removed surface-prune top-level names', () => {
    for (const name of ['login', 'logout', 'budget', 'bench', 'mine', 'cost', 'output', 'profiles', 'snapshot', 'cp', 'resume', 'roster', 'status', 'tickets', 'alias', 'inbox', 'apply', 'beta']) {
  ...
  it('keeps pruned names in RETIRED so distance-1 typos do not auto-correct into live commands', async () => {
    ...
    for (const name of ['login', 'logout', 'budget', 'bench', 'mine', 'cost', 'output', 'profiles', 'snapshot', 'cp', 'webhook', 'resume', 'roster', 'status', 'tickets', 'alias', 'inbox', 'apply', 'beta']) {
