...
export function listBlocks(root?: string): OpenBlock[] {
  ...
  for (const name of names.filter(n => n.endsWith('.json')).sort()) {
    try {
      ...
      if (parsed.blockId && parsed.sessionId && parsed.question) {
 ...
 * Embedded so it ships with the compiled CLI and can be installed to the system
 * hooks dir without a separate file in the npm tarball.
 */
...
def main():
    ...
    runtime = os.environ.get("AGENTS_RUNTIME", "claude")

    block_id = f"block-{session_id}"
    block = {
        "blockId": block_id,
...
/**
 * Install the feed-publish hook script into the system hooks dir and add its
 * manifest entry to the system agents.yaml. Idempotent -- skips if the script
 * is already present and up to date.
 */
export function ensureFeedPublishHook(): { installed: boolean; error?: string } {
  try {
    const home = process.env.HOME ?? os.homedir();
    const hooksDir = path.join(home, '.agents', '.system', 'hooks');
    const scriptPath = path.join(hooksDir, '10-feed-publish.py');

    if (fs.existsSync(scriptPath)) {
      const existing = fs.readFileSync(scriptPath, 'utf-8');
      if (existing === FEED_PUBLISH_HOOK_SCRIPT) return { installed: false };
    }

    fs.mkdirSync(hooksDir, { recursive: true });
    fs.writeFileSync(scriptPath, FEED_PUBLISH_HOOK_SCRIPT, { mode: 0o755 });

    const agentsYamlPath = path.join(home, '.agents', '.system', 'agents.yaml');
    let doc: Record<string, any> = {};
    if (fs.existsSync(agentsYamlPath)) {
      try {
        doc = yaml.parse(fs.readFileSync(agentsYamlPath, 'utf-8')) ?? {};
      } catch { /* start fresh */ }
    }
    if (!doc.hooks) doc.hooks = {};
    if (!doc.hooks['feed-publish']) {
      doc.hooks['feed-publish'] = {
        agents: ['claude'],
        events: ['PreToolUse'],
        matcher: 'AskUserQuestion',
        script: '10-feed-publish.py',
      ...
      fs.writeFileSync(agentsYamlPath, yaml.stringify(doc));
    }
