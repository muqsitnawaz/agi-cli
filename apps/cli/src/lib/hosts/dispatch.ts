...
export interface DispatchOptions {
  ...
  strategy?: string;
  /** Logical account label to forward as `--account <label>`; the remote resolves it against ITS bindings (issue #2300). */
  account?: string;
  balanced?: boolean;
  fallback?: string;
  mode?: string;
...
export interface InteractiveDispatchOptions {
  ...
  strategy?: string;
  /** Logical account label to forward as `--account <label>`; the remote resolves it (issue #2300). */
  account?: string;
