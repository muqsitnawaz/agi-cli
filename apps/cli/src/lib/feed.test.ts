...
describe('feed store', () => {
  ...
  it('installs the hook without discarding existing YAML comments', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-feed-install-'));
    const systemDir = path.join(home, '.agents', '.system');
    fs.mkdirSync(systemDir, { recursive: true });
    const agentsYaml = path.join(systemDir, 'agents.yaml');
    fs.writeFileSync(agentsYaml, 'hooks:\n  # keep this comment\n  existing:\n    agents: [claude]\n    events: [Stop]\n    script: existing.sh\n');
    const originalHome = process.env.HOME;
    process.env.HOME = home;
    try {
      expect(ensureFeedPublishHook()).toEqual({ installed: true });
      expect(ensureFeedPublishHook()).toEqual({ installed: false });
    } finally {
    ...
    expect(fs.readFileSync(path.join(systemDir, 'hooks', '10-feed-publish.py'), 'utf-8')).toBe(FEED_PUBLISH_HOOK_SCRIPT);
  });
});
