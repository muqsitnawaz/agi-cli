...
describe('workspaceTargetsForDef', () => {
  ...
  it('probes the checkout ROOT, never a defaultPath subdir or a repo subpath', () => {
    // The probe resolves `.git` at the exact path, so it must target the repo
    // root — probing the `defaultPath` working subdir (no `.git`) would report a
    // healthy monorepo project `✗ missing`.
    expect(workspaceTargetsForDef({
      name: 'rush',
      root: '~/src/rush',
      defaultPath: '~/src/rush/apps/web',
      repos: [{ slug: 'o/infra', path: '~/src/infra', subpath: 'deploy' }],
    })).toEqual(['~/src/rush', '~/src/infra']);
  });
