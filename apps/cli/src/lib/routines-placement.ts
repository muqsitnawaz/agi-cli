...
export function pickFleetDevice(
  ...
  if (candidates.length === 0) {
    ...
    // without a registry entry still runs locally. Only do this when no platform
    // filter was requested or when we don't know our own platform (no registry
    // means we can't verify it); otherwise an unmet platform filter should fail
    // loud so e.g. `fleet/linux` never silently lands on a macOS box.
