...
| Gemini † | >= 0.26.0 | yes | no | yes | yes (.toml) | >= 0.8.0 | >= 0.36.0 | `GEMINI.md` | no |
...
Permissions sync is gated on the `allowlist` capability (Claude, Codex >= 0.138.0, Antigravity, Grok, Kimi, Kiro 2.8.0+, OpenCode, Goose, and Droid >= 0.57.5). **Host CLIs** (`agents cli`) are agent-agnostic PATH binaries — not in this matrix. Install paths call `supports(agent, cap, version)` before writing; gated capabilities skip with a clear reason instead of silently ignored config.
