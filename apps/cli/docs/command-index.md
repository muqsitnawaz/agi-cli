...
## snapshot — One-process poll snapshot: install inventory + active and resumable sessions (optional feed/sync). Not the sync-status command — use `agents status` for drift.

```
agents snapshot  One-process poll snapshot: install inventory + active and resumable sessions (optional feed/sync). Not the sync-status command — use `agents status` for drift.
```
