...
agents feed answer <attention-key>  Answer an open attention item — atomically claim it and route the reply to the waiting agent
agents feed post <text...>          Post a status update to the fleet activity stream (for agents)
agents feed watch                   Stream the reconciled operator projection (agents + attention + activity) as NDJSON
```
