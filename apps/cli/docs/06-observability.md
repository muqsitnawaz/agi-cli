...
- **Kimi and Droid usage never refresh the token.** `agents view` is a
  ...
  (rewriting the file, invalidating the refresh token, racing a running `kimi` /
  `droid`). An expired token simply falls back to the cached snapshot; each
  agent's own CLI refreshes on its next launch (Droid's token lives 24h). Droid
  surfaces the `standard` (primary) rate-limit pool, not the free `core`
  fallback pool.
