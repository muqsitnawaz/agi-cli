...
`--host <device>` (alias: `--device`) routes a `routines` subcommand to a remote
...
When you try to run a job on a host outside its allowlist, the CLI prints the
allowed device names and a `--host <device>` hint:

```
Job 'drain' is restricted to device(s) yosemite-s0, mac-mini and never runs here.
  Run it on one of them: agents routines run drain --host <device>
```
...
| `normalizeDevices()` | routines.ts | Deduplicate and normalize a devices array |
| `jobDeviceOwners()` | routines.ts | Return device names that own a job (for error messages) |
