...
- **SING-1 (MUST).** Every fleet-affecting capability MUST have exactly one scheduler
  ...
  (`lib/watchdog/rotate.ts`). The watchdog, device-probe, tmux-reconcile,
  launch-health, fleet-cache-warm, session-cache-warm, usage-refresh, and
  auto-dispatch ticks (RUSH-2353) were formerly hardcoded `setInterval`s inside
  `runDaemon()` — a second, unowned scheduling path duplicating what routines
  already provide (declaration, run history, pause, device pin). They are now
  shipped system routines (`gh:phnx-labs/.agents-system` `routines/*.yml`)
  whose `command:` invokes the migrated tick body (`lib/daemon-ticks.ts`) via
  `agents __daemon-tick <name>`, fired by the same pid-claimed `JobScheduler`
  as every other routine — one scheduler, one executor, same as before, minus
