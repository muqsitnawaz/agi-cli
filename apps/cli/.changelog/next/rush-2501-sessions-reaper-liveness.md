- **Dead tmux sessions are now reaped automatically every 5 minutes.** Sessions
  ...
  with a live pane is never touched. Reaping is daemon-owned with no new command
  surface — the existing `agents tmux kill <name>` (idempotent) covers the manual
  case. Source: `apps/cli/src/lib/tmux/session.ts`, `apps/cli/src/lib/daemon.ts`.
