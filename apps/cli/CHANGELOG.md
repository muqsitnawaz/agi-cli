...
- **Session-picker clients no longer reconstruct resumability from two CLI listings (RUSH-2484).** `agents sessions resume --candidates --json` now emits the canonical fleet-wide recent + live union with stable identity, lifecycle state (`detached`, `background`, `parked`, `inactive`, or `watched`), viewer, source host, last activity, deterministic ordering, and the CLI recovery invocation for each row. `--abandoned-only` removes rows already watched elsewhere. Factory can render this payload directly instead of joining `sessions --all --json` to `sessions --active --json` or rebuilding remote recovery commands.
