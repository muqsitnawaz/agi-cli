...
- **Factory and other UI clients can consume canonical harness, account eligibility, and quota freshness from CLI JSON (RUSH-2484).** `agents harness list --json` now returns every native harness's `cliCommand`, run modes, and capability declaration directly from `AGENTS`; consumers no longer need a checked-in agent registry mirror. `agents devices harnesses --json` now stamps each eligibility verdict, preserves `AccountInfo.usageStatus` including `out_of_credits`, and exposes the quota snapshot timestamp plus an explicit `unavailableReason`. Auto placement's local and remote probes use that canonical eligibility: an ineligible device is excluded rather than merely sorted lower, an all-ineligible pool fails loud, and a probe/placement error no longer silently launches locally. Launch remains atomic through `agents run <agent> --where auto`; any picker preview is advisory and must not be converted into a later `--host` launch. Source: `apps/cli/src/commands/harness.ts`, `apps/cli/src/lib/devices/harness-inventory.ts`, `apps/cli/src/lib/hosts/ready.ts`, `apps/cli/src/lib/teams/{placement-probe,scheduler}.ts`, `apps/cli/src/lib/smart-launch.ts`.
