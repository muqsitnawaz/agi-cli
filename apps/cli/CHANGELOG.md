...
- **Factory and other UI clients can consume canonical harness, account eligibility, and quota freshness from CLI JSON (RUSH-2484).** `agents harness list --json` now returns every native harness's `cliCommand`, run modes, and capability declaration directly from `AGENTS`; consumers no longer need a checked-in agent registry mirror. `agents devices harnesses --json` now stamps each eligibility verdict and exposes the quota snapshot timestamp plus an explicit `unavailableReason`. Auto placement's existing one-shot remote view probe now treats a signed-in but rate-limited or out-of-credits account as ineligible, using the same `usageStatus` emitted by `agents view --json`. Launch remains atomic through `agents run <agent> --where auto`; any picker preview is advisory and must not be converted into a later `--host` launch. Source: `apps/cli/src/commands/harness.ts`, `apps/cli/src/lib/devices/harness-inventory.ts`, `apps/cli/src/lib/hosts/ready.ts`, `apps/cli/src/lib/teams/placement-probe.ts`.
