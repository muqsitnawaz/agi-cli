...
- **Fix: daemon no longer crash-loops when started from the bare `browser` or `computer` shim.** Daemon launch resolution now maps sibling JavaScript shims to the package's main `index.js` entrypoint before generating launchd/systemd commands, and fails clearly if that invariant is broken instead of launching a binary without the `daemon` command. Source: `apps/cli/src/lib/daemon.ts` (`getAgentsBinPath`). (RUSH-1527)
