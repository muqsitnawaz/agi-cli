...
- **Fix: daemon no longer crash-loops when started from the bare `browser` or `computer` shim.** Daemon launch resolution now maps sibling JavaScript shims to the package's main `index.js` entrypoint before generating launchd/systemd commands, and fails clearly if that invariant is broken instead of launching a binary without the `daemon` command. Headless auto-start also reads the long-lived Claude token only from an already-unlocked secrets-agent snapshot, so it cannot hang forever on a biometric prompt nobody can answer; an interactive start can still prompt normally. Source: `apps/cli/src/lib/daemon.ts` (`getAgentsBinPath`, `readDaemonClaudeOAuthToken`), `apps/cli/src/lib/secrets/bundles.ts` (`agentOnly`). (RUSH-1527)
