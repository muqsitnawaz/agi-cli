...
- **Factory fleet reads now use the owning `agents devices` JSON surfaces (RUSH-2484).** `agents devices list --json` returns each effective device profile/config plus canonical health/headroom, while `agents devices accounts --json` preserves account quota status including `out_of_credits`, capture time, and unavailable reason. Auto placement uses the same eligibility: an ineligible device is excluded rather than merely sorted lower, an all-ineligible pool fails loud, and a probe/placement error no longer silently launches locally. Launch remains atomic through `agents run <agent> --where auto`; picker reads are advisory and must not be converted into a later `--host` launch. Source: `apps/cli/src/commands/ssh.ts`, `apps/cli/src/lib/devices/harness-inventory.ts`, `apps/cli/src/lib/hosts/ready.ts`, `apps/cli/src/lib/teams/{placement-probe,scheduler}.ts`, `apps/cli/src/lib/smart-launch.ts`.
