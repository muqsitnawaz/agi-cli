...
- **`agents snapshot --json` is the canonical Factory fleet read model (RUSH-2484).** `FleetSnapshot` now includes the native harness catalog directly from `AGENTS` plus one row per device with its effective profile/config, installed harness accounts, quota eligibility, verdict timestamp, and structured freshness/unavailable reason. `AccountInfo.usageStatus` is preserved, including `out_of_credits`; clients no longer parse `agents.yaml`, mirror the agent registry, or join device/view/usage outputs. Auto placement's local and remote probes use the same eligibility: an ineligible device is excluded rather than merely sorted lower, an all-ineligible pool fails loud, and a probe/placement error no longer silently launches locally. Launch remains atomic through `agents run <agent> --where auto`; any picker preview is advisory and must not be converted into a later `--host` launch. Source: `apps/cli/src/lib/snapshot.ts`, `apps/cli/src/lib/devices/harness-inventory.ts`, `apps/cli/src/lib/hosts/ready.ts`, `apps/cli/src/lib/teams/{placement-probe,scheduler}.ts`, `apps/cli/src/lib/smart-launch.ts`.
