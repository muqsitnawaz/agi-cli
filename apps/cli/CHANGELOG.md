...
- **Fix: `agents message` now delivers to non-Claude teams teammates whose runtime session id differs from the teams `agentId`.** The teams spawn path (`buildCommand`) only passed `--session-id <agentId>` for Claude, so non-Claude teammates (Codex, Gemini, etc.) got a mailbox keyed by an auto-generated session UUID while the message sender resolved to the `agentId` — messages were enqueued to a directory the agent never drained. `--session-id` is now passed for all agent types, so `AGENTS_MAILBOX_DIR` is always keyed by the canonical `agentId`. Source: `apps/cli/src/lib/teams/agents.ts` (`buildCommand`), `apps/cli/src/lib/mailbox-target.ts`. (RUSH-1534)
