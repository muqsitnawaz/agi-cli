...
- **Routines now validate their execution context before activation and fire once per schedule slot (RUSH-2290).** A routine may select one execution `project` plus a portable `cwd`; relative paths resolve on the execution device, including Linear-imported projects without a checkout binding. Add/edit save operational blockers paused with structured readiness, resume rechecks them, and raw YAML edits are transactional. Durable UTC slot claims and all-entry-point active-run claims prevent duplicate and overlapping launches. Routine history records blocked/skipped/pre-spawn attempts even when no session transcript exists, and the menu bar exposes concrete readiness/history without becoming a scheduler. `projects[]` remains grouping metadata and `repo` remains external Git/cloud/webhook identity.
