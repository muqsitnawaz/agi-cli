...
- **Factory fleet readiness now comes from the existing device/account JSON contracts (RUSH-2484).** `agents devices list --json` includes effective config plus resource health/headroom; `agents devices status --json` includes each row's effective profile/config; and `agents devices accounts --json` exposes the quota verdict, capture/reset timestamps, and explicit unavailable reason, including the distinct `out_of_credits` verdict. Automatic `agents run ... --device auto` placement excludes unreachable, overloaded, signed-out, rate-limited, and out-of-credits choices and fails loud when placement cannot be evaluated instead of silently launching locally. No snapshot or new consumer-specific command is involved. Source: `apps/cli/src/commands/{ssh,exec}.ts`, `apps/cli/src/lib/{smart-launch,hosts/ready}.ts`, `apps/cli/src/lib/devices/harness-inventory.ts`.
