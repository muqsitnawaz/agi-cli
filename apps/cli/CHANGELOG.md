...
- **`agents sessions --active` now distinguishes dead and abandoned sessions
  ...
  `running` as its honest floor. The default list and grouped active tallies render
  `closed` / `abandoned` distinctly, and Factory maps `closed` to done and
  `abandoned` to failed so dead work no longer appears idle. Source:
