...
- **Factory now reads teams, ticket queues, and Watchdog playbooks through canonical agents-cli nouns instead of reimplementing them in the extension (RUSH-2484).** `agents teams list --json` includes canonical teammate details and accepts `--cwd`; `agents tickets list --json` aggregates the workspace's Linear and GitHub work items; and `agents watchdog playbook --ensure --json` owns the playbook path, scaffold, and status. Factory only invokes these JSON/verb surfaces and maps their output to VS Code views. Source: `apps/cli/src/commands/{teams,tickets,watchdog}.ts`, `apps/cli/src/lib/tickets/list.ts`, `apps/factory/src/vscode/{swarm,foreman.sources,tasks,watchdog}.vscode.ts`.
