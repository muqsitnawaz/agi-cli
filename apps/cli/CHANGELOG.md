...
- **Wire allowlist support for Kiro CLI.** Kiro CLI (`kiro-cli` ≥ 2.8.0) reads declarative `~/.kiro/settings/permissions.yaml` rules. agents-cli now flips Kiro's `allowlist` capability, converts canonical permission groups to Kiro `capability`/`effect`/`match` rules, and writes them into the versioned home. Bash maps to `shell`, Read/Grep/Glob to `fs_read`, Write/Edit to `fs_write`, WebFetch to `web_fetch`, WebSearch to `web_search`, and MCP/Subagent/Skill to their named capabilities. Source: `apps/cli/src/lib/agents.ts`, `apps/cli/src/lib/permissions.ts` (Kiro converter), `apps/cli/src/lib/staleness/detectors/permissions.ts`, `apps/cli/src/lib/resources/permissions.ts`. (RUSH-1392)
