...
- **`agents snapshot --json` now includes canonical resumable-session rows (RUSH-2484).** `resumableSessions` joins recent transcripts with the snapshot's single live gather, then emits stable identity, lifecycle state (`detached`, `background`, `parked`, `inactive`, or `watched`), the canonical `unwatched` picker predicate, viewer, source host, last activity, deterministic ordering, and the `agents sessions resume` recovery invocation. Factory can render this payload directly instead of joining `sessions --all --json` to `sessions --active --json`, classifying rows, or rebuilding remote recovery commands. `agents sessions resume` remains action-only; no consumer-specific query flags were added.
