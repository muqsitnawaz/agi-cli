...
- **`agents feed` lists open blocks -- decisions agents are waiting on.** A new outbound feed store mirrors the inbound mailbox pattern: when a top-level agent calls `AskUserQuestion`, a `PreToolUse` hook (`10-feed-publish.py`) writes one block record per session to `~/.agents/.history/feed/`. `agents feed` aggregates those records locally (and across hosts with `--host`), showing the question text, options, age, and a ready-to-paste `agents message` reply command. `--json` emits structured output. Only top-level agents publish; Task/Agent subagents are gated out so a swarm of subagents never floods the feed. The hook is fail-open with a 5-second timeout so a feed hiccup never blocks a tool call. `ensureFeedPublishHook()` installs the hook idempotently into the system hooks dir. Source: `apps/cli/src/lib/feed.ts`, `apps/cli/src/commands/feed.ts`, `apps/cli/src/lib/state.ts` (`getFeedDir`). (RUSH-1473)
