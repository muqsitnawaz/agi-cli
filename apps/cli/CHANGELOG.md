...
- **Fix: daemon no longer crash-loops when started from the bare `browser` or `computer` shim.** The daemon launch resolution used `process.argv[1]` verbatim, so when the daemon was auto-started as a side-effect of a sibling shim (`browser start` triggering `startDaemon` via IPC), it spawned `node dist/browser.js daemon _run` — which the browser shim doesn't handle, producing a crash that launchd's `KeepAlive` would restart indefinitely. `getAgentsBinPath` now detects sibling JS entrypoints and resolves to the sibling `index.js` (the main CLI entry) so the daemon always launches through the full CLI. Source: `apps/cli/src/lib/daemon.ts` (`getAgentsBinPath`). (RUSH-1527)
