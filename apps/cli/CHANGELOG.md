...
- **`agents sessions watch --json` is the canonical incremental session-state stream for Factory and other long-lived consumers (RUSH-2484).** It emits versioned NDJSON `reset`, `upsert`, `remove`, device-scope availability, and heartbeat envelopes with opaque row keys and monotonic per-stream sequence numbers. Rows include CLI-owned recovery metadata, and unavailable device scopes retain their last known recovery rows instead of disappearing. `--local` disables fleet aggregation. Startup reads one shared snapshot; steady state tails row deltas appended by the canonical cache writer, so watcher ticks never trigger another full live-session or fleet gather. Source: `apps/cli/src/commands/sessions-watch.ts`, `apps/cli/src/lib/session/watch.ts`, `apps/cli/src/lib/session/session-cache.ts`.
