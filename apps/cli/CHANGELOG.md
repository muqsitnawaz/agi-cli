...
- **Repeated full session listings reuse transcript membership (RUSH-2318).** The process caches the session index's path-existence sweep and invalidates it on same-process index writes or SQLite `data_version` changes from concurrent writers, avoiding hundreds of repeated directory reads without hiding newly indexed sessions. Source: `apps/cli/src/lib/session/db.ts`.
