...
- **`--host` (fleet device routing flag) is removed; `--device` / `-D` is now the sole routing flag (RUSH-2494).** Every command that accepted `-H`/`--host` as a fleet device target now accepts only `-D`/`--device`. The `agents hosts` command group (noun) is untouched. Internal identifiers — `host`, `hostStrategy`, PlacementMode fields — are also unchanged. If you have scripts using `--host <device>` for routing, replace them with `--device <device>`. The stripped-flag path in `HOST_ROUTING_SPECS` retains a backwards-compat strip for `--host` in forwarded argv so remote dispatch does not break on mixed-version fleets, but the flag is no longer accepted at the user-facing CLI layer.
