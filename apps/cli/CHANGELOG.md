...
- **`agents sessions --all --json` now emits the complete historical + live session model (RUSH-2484).** The existing listing folds every live session into its durable rows, then adds lifecycle state (`detached`, `background`, `parked`, `inactive`, or `watched`), canonical `resumable` / `unwatched` predicates, viewer, source device, last activity, deterministic ordering, and the `agents sessions resume` recovery invocation. Factory can render the same session rows directly instead of joining two commands, classifying state, or rebuilding recovery commands. Session data remains under `agents sessions`; `agents snapshot` is unchanged, and `agents sessions resume` remains action-only.
