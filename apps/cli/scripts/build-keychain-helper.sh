...
[ -f "$PROFILE" ] || { echo "Missing $PROFILE. It is a committed input (see commit 2567004b4) -- do NOT regenerate it at developer.apple.com. Recover it from git history and verify apps/cli/bin/embedded.provisionprofile is tracked on origin/main, then either re-run from an up-to-date checkout or rerun via release.sh, whose home-base phase recovers it from origin/<default> automatically." >&2; exit 1; }
