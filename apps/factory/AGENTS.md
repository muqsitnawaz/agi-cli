...
Factory's auto-host selection reads per-device enable/prefer flags managed by the CLI (`agents devices config <name> auto-launch.enabled|auto-launch.preferred on|off`; the retired `agents devices enable|disable|prefer|unprefer <name>` forward there). The store is the tracked per-device doc `~/.agents/devices/<name>/agents.yaml` (`config:` block), layered over the fleet-wide defaults in central `~/.agents/agents.yaml` `fleet.defaults.config`.
